...
echo "$0: every printer provisioned onto it sends its token, its files, and the WiFi SSID and"
echo "$0: PrusaLink password it reports in its own INFO, across the network in clear - and plain HTTP"
echo "$0: has no integrity, so gcode and commands can be altered in flight rather than only read."
echo "$0: This is a LAN proposition: publish it to the internet knowingly or not at all."
echo "$0: Printers whose firmware can load a custom certificate should be re-provisioned onto the"
echo "$0: TLS listener instead; unset LEGACY_PRINTER_PORT to close it."
