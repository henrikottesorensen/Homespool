...
public class SourceFileEncodingTests
{
    ...
    private static readonly HashSet<string> SkippedDirectories = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "firmware", "logs", "notes", "private-captures", "PrinterService.Mvc",
    };
    ...
    private static DirectoryInfo RepositoryRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "PrinterService.slnx")))
        {
            directory = directory.Parent;
        }

        return directory
            ?? throw new InvalidOperationException($"No PrinterService.slnx above {AppContext.BaseDirectory}.");
    }
    ...
    public void TheScanActuallyFindsTheSourceTree()
    {
        ...
        files.Should().Contain(f => Relative(f) == Path.Combine("PrinterService.Host", "Program.cs"),
