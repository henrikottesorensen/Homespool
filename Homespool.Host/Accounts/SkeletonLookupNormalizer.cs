...
/// <summary>
/// The lookup key for a username is Squint's lookalike key, upper-cased: two names that look alike
...
/// With the lookalike key as the key, a Cyrillic <c>а</c> in <em>henrik</em>, <em>þor</em> beside
...
/// </para>
/// <para>
/// <b>Upper-cased last</b>, because the key maps some capitals to lower-case prototypes (<c>I</c>
/// becomes <c>l</c>) and the stored key must not depend on which case was typed. The key comes from
/// Squint's own tables rather than the machine's ICU, so it is the same on every platform this runs
/// on, which a stored unique key has to be.
/// </para>
/// <para>
/// <b>A stored key is only as current as the Unicode data that made it.</b> A Squint upgrade with new
/// confusable data can change a name's key, so every user's key is recomputed at start-up - see
/// <see cref="UsernameKeyRefresh"/> - rather than trusted from the row.
/// </para>
/// <para>
/// Role names go through <see cref="NormalizeName"/> as well, because Identity has one normaliser for
/// both. The one role, <c>Admin</c>, is its own key, so its stored value is unchanged.
/// </para>
/// </remarks>
public sealed class SkeletonLookupNormalizer : ILookupNormalizer
{
    /// <summary>The upper-cased lookalike key of <paramref name="name"/>.</summary>
    public string? NormalizeName(string? name)
    {
        ...
        return Lookalikes.Key(name).ToUpperInvariant();
    }
