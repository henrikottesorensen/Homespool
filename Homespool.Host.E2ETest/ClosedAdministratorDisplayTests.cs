...
/// banner and the detailed health report - and that a closed administrator's live cookie stops
/// being shown any of it, though it still carries the role until its stamp is next re-checked.
/// </summary>
/// <remarks>
...
public sealed class ClosedAdministratorDisplayTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task AClosedAdministratorsLiveSessionIsShownNothingAnAdministratorIs()
    {
        // Arrange
        ...
        using (deputyClient)
        {
            ...
            // Act
            string deputyPage = await GetAsync(deputyClient, "/Printers");
            ...
            deputyPage.Should().NotContain(AdminMenu, "a closed administrator's cookie still carries the role, and the menu must not believe it");
            deputyPage.Should().NotContain(InsecureBanner, "nor the health banner");
            deputyHealth.Should().NotContain("\"checks\"", "nor the detailed report, which runs every check on demand")
