...
public class PrusaConnectPrinterAuthenticationHandler : AuthenticationHandler<PrusaConnectAuthenticationSchemeOptions>
{
    ...
    /// it is written, not only here. Nothing cleans a header on the way in, so every site that logs
    /// one does it itself: this, the websocket connect line in <c>PrusaConnectPrinterController</c>,
    /// and <c>PrinterClient</c> for the user agent.
    /// </para>
    /// </remarks>
