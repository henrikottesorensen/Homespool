...
/// </para>
/// <para>
/// <b>Same registrations, same order, same lifetimes.</b> The one thing the framework does that this
/// cannot is walk the type hierarchy to pick a store: <c>AddEntityFrameworkStores</c> reflects over the
/// context to find the six framework entity types, where <see cref="AddHomespoolStores"/> simply names
/// the ones <see cref="HomespoolDbContext"/> inherits. A change to that base class therefore has to be
...
public static class IdentityServices
{
    ...
    public static IdentityBuilder AddHomespoolIdentity(this IServiceCollection services, Action<IdentityOptions> configure)
    {
        ...
        return new IdentityBuilder(typeof(HSUser), typeof(IdentityRole<long>), services);
    }
