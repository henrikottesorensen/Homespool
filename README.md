...
| `MaxIncomingMessageBytes` | `1048576` | Bytes a printer may accumulate without completing a message before the connection is closed. Guards the receive buffer, which otherwise has no ceiling at all. **Roughly eleven times the largest message ever measured** (92 831 bytes — a `FILE_INFO` carrying a base64 thumbnail), and the headroom is deliberate: that figure is a sample rather than a limit, since preview size is a slicer setting and a directory listing grows with the files on the drive. Tripping it logs the printer and the byte count, because the symptom is a dropped connection and nothing else would tell it from bad wifi. |
