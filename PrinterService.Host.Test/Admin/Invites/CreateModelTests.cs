...
public sealed class CreateModelTests : IDisposable
{
        ...
        PSDbContext context, bool signInAdmin = true)
    {
        ...
        PSUser admin = new() { UserName = "admin@example.com", Email = "admin@example.com", EmailConfirmed = true };
    ...
    public async Task OnPostAsyncCreatesANewAccountInviteAndMailsTheAcceptLink()
    {
        // Arrange
        await using PSDbContext context = await MigratedContextAsync();
        (CreateModel model, PSUser admin, CapturingEmailSender emailSender) = await NewModelAsync(context);
