...
public sealed class LocalSignIn
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>The proof goes with the session</b>, and it is cleared here rather than on the logout page
    /// so that every route out of a session takes it. Otherwise signing out and back in inside the
    /// window would walk past every gate without proving anything - which is exactly the
    /// shared-machine case the window is there to bound.
    /// </remarks>
