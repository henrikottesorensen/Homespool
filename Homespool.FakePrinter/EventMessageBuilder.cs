...
public static class EventMessageBuilder
{
    ...
    /// <summary>
    /// <c>JOB_INFO</c> - what a <c>SEND_JOB_INFO</c> is answered with, and what a successful
    /// <c>START_PRINT</c> is answered with too (planner.cpp:728).
    /// </summary>
    /// <param name="state">The wire device state.</param>
    /// <param name="jobId">The job being described, rendered at the <b>root</b> rather than in
    /// <c>data</c>.</param>
    /// <param name="path">
    /// The file being printed, or null for a job the printer only remembers.
    /// </param>
    /// <param name="jobState">
    /// The <i>job</i>'s own state - <c>PRINTING</c> while it runs, or <c>FIN_OK</c>/<c>FIN_STOPPED</c>
    /// afterwards. A different vocabulary from <paramref name="state"/>, which is the machine's.
    /// </param>
    /// <param name="commandId">The command being answered.</param>
    /// <remarks>
    /// <para>
    /// <b>The shape is firmware's own, taken from its render fixtures</b>
    /// (<c>render-fixtures.json</c>, generated by <c>connect_render_dump</c>): a running job renders
    /// <c>state</c>, <c>display_name</c> and <c>path</c>; a job the printer merely remembers renders
    /// <c>state</c> alone. That second shape matters as much as the first - it is what stops a
    /// consumer treating "there was such a job" as "that job is yours".
    /// </para>
    /// <para>
    /// The <b>no 8.3 aliasing</b> deviation described on <see cref="BuildFileInfo"/> applies here in
    /// the same way, and for the same reason.
    /// </para>
    /// </remarks>
    public static byte[] BuildJobInfo(string state,
                                      int jobId,
                                      string? path,
                                      string jobState = "PRINTING",
                                      uint? commandId = null)
    {
        ArrayBufferWriter<byte> buffer = new();
        ...
        using (Utf8JsonWriter writer = new(buffer))
        {
            ...
            writer.WriteStartObject("data");
            writer.WriteString("state", jobState);
            ...
            writer.WriteString("event", "JOB_INFO");
            writer.WriteEndObject();
        }
