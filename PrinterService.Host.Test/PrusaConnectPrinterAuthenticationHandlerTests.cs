...
public sealed class PrusaConnectPrinterAuthenticationHandlerTests : IDisposable
{
    ...
    /// </summary>
    private async Task<AuthenticateResult> AuthenticateAsync(PSDbContext context,
                                                             string? fingerprint,
                                                             string? token,
    ...
    public async Task AnEnrolledPrinterAuthenticatesAndCarriesItsIdentityInTheTicket()
    {
        ...
        result.Principal.FindFirst(PsClaimTypes.PrinterId)!.Value.Should().Be($"{printer.Id}");
        result.Principal.FindFirst(PsClaimTypes.Owner)!.Value.Should().Be($"{printer.TeamId}");
    }
    ...
    public async Task FirstContactWithAProvisionedTokenEnrollsThePrinter()
    {
        ...
        await using PSDbContext verify = NewContext();
    ...
    {
        // Arrange
        await using PSDbContext first = await MigratedContextAsync();
        ...
        await using PSDbContext second = NewContext();
    ...
    public async Task AProvisionedTokenBindsOnlyItsOwnPrinter()
    {
        ...
        // Assert
        result.Succeeded.Should().BeTrue();
        result.Principal!.FindFirst(PsClaimTypes.PrinterId)!.Value.Should().Be($"{target.Id}");
    ...
    {
        // Arrange
        await using PSDbContext seed = await MigratedContextAsync();
        (Printer printer, string token) = await AddProvisionedPrinterAsync(seed);

        await using PSDbContext left = NewContext();
        await using PSDbContext right = NewContext();
