...
public sealed class PrintFileCatalog
{
        ...
        CancellationToken cancellationToken, string? displayName = null)
    {
        PublishedFile published =
            await _store.SaveAsync(userId, fileName, content, overwrite, cancellationToken, displayName);
    ...
    public async Task<StoredFile?> PublishAsync(long userId, string token, bool overwrite,
        CancellationToken cancellationToken, string? displayName = null)
    {
        PublishedFile? published = _store.Publish(userId, token, overwrite, displayName);
