...
public static class ForwardedHeadersConfigurator
{
    ...
    public static void Apply(XForwardedOptions source, ForwardedHeadersOptions target, Action<string>? onIgnoredEntry = null)
    {
        ...
        // those links name is Request.Host, and the Host header already carries the address the user
        // reached: the proxy passes the browser's own name through, and host filtering refuses any
        ...
        // that rewrites the NAME is therefore unsupported; host filtering answers it with a 400, so
        // the failure is on the first page rather than in somebody's inbox.
        //
        // The port on that header is the proxy's to set, and the shipped one does: host filtering
        // matches names and never ports, so a port taken from the request is a value nothing checks
        // and whoever sent the request would choose the one in a link mailed to somebody else.
