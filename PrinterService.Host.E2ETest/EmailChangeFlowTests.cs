...
public sealed class EmailChangeFlowTests : IAsyncLifetime, IDisposable
{
    ...
    {
        // Arrange
        (PSUser _, HttpClient client) = await EnrollmentFlowHelper.CreateAuthenticatedUserAsync(_factory, "changer@example.com");
    ...
    {
        // Arrange
        (PSUser user, HttpClient client) = await EnrollmentFlowHelper.CreateAuthenticatedUserAsync(_factory, "before@example.com");
        ...
        using (client)
        {
            ...
            using IServiceScope scope = _factory.Services.CreateScope();
            UserManager<PSUser> users = scope.ServiceProvider.GetRequiredService<UserManager<PSUser>>();
            PSUser reloaded = (await users.FindByIdAsync(user.Id.ToString()))!;
    ...
    {
        // Arrange
        (PSUser _, HttpClient client) = await EnrollmentFlowHelper.CreateAuthenticatedUserAsync(
    ...
    private async Task<string> BuildConfirmUrlAsync(long userId, string newEmail)
    {
        using IServiceScope scope = _factory.Services.CreateScope();
        UserManager<PSUser> users = scope.ServiceProvider.GetRequiredService<UserManager<PSUser>>();
        PSUser user = (await users.FindByIdAsync(userId.ToString()))!;
