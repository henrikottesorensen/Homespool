...
public sealed class ClaimModelTests : IDisposable
{
    ...
    private static async Task<(ClaimModel model, PSUser user)> NewModelAsync(PSDbContext context, string email = "owner@example.com")
    {
        ...
        PSUser user = new() { UserName = email, Email = email, EmailConfirmed = true };
    ...
    private static async Task<string> SeedClaimableCodeAsync(PSDbContext context, string fingerprint)
    {
    ...
    public async Task OnGetAsyncListsOnlyTeamsTheUserCanManage()
    {
        ...
        (ClaimModel model, PSUser user) = await NewModelAsync(context);
